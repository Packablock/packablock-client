 ...
 */
export function parseSemVerConstraint(constraint: string, currentPinned: string): SemVerRange {
...
export function renderCandle(
  ...
  const vMax = isInfinity 
    ? Math.max(vLatest, vPinned) + (Math.max(vLatest, vPinned) - vMin || 1000) * 1.5
  ...
  const idxMax = isInfinity ? width - 1 : Math.max(idxLatest, Math.min(width - 1, scale(vMax)));

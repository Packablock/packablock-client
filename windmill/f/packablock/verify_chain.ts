...
export async function main(
  ...
  if (!res.ok) {
    throw new Error(`Failed to pull chain from server: ${res.status} ${res.statusText}`);
  }
  
  const chainContent = await res.text();
  console.log(`Successfully retrieved chain content. Length: ${chainContent.length} bytes.`);

...
// 8. Update client CLI source code version strings
...
if (fs.existsSync(cliPath)) {
	...
	console.log("📝 src/cli.ts CLI version updated.");
}

const apiPath = path.join(process.cwd(), "src/api.ts");
if (fs.existsSync(apiPath)) {
	let apiContent = fs.readFileSync(apiPath, "utf8");
	apiContent = apiContent.replace(
		/headers\["X-Client-Version"\] = "[^"]+"/,
		`headers["X-Client-Version"] = "${nextVersion}"`,
	);
	fs.writeFileSync(apiPath, apiContent, "utf8");
	console.log("📝 src/api.ts X-Client-Version header updated.");
}

// 9. Git Add, Commit, Tag and Push
const filesToStage = ["package.json", "CHANGELOG.md"];
if (fs.existsSync(cliPath)) {
	filesToStage.push("src/cli.ts");
}
if (fs.existsSync(apiPath)) {
	filesToStage.push("src/api.ts");
}
